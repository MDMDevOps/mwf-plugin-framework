...
class DateTimeHelper
{
    ...
    public static function time_ago($from, $to = null, $format_past = null, $format_future = null)
    {
        ...
        $to = $to ?? \time();
        $to = \is_numeric($to) ? new DateTimeImmutable('@' . $to, \wp_timezone()) : new DateTimeImmutable($to, \wp_timezone());
        $from = \is_numeric($from) ? new DateTimeImmutable('@' . $from, \wp_timezone()) : new DateTimeImmutable($from, \wp_timezone());
        if ($from < $to) {
