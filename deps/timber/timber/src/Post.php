...
class Post extends CoreEntity implements DatedInterface, Setupable
{
    ...
    public function __get($field)
    {
        ...
        if ('_thumbnail_id' === $field) {
            Helper::doing_it_wrong("Accessing the thumbnail ID through {{ {$this->object_type}._thumbnail_id }}", "You can retrieve the thumbnail ID via the thumbnail object {{ {$this->object_type}.thumbnail.id }}. If you need the id as stored on this post's postmeta you can use {{ {$this->object_type}.meta('_thumbnail_id') }}", '2.0.0');
        }
    ...
    {
        $can = [\get_post_type_object($query->queried_object->post_type)->cap->edit_post];
    ...
    {
        $data = \array_merge($data, ['slug' => $this->wp_object->post_name, 'status' => $this->wp_object->post_status]);
        return $data;
    }
    /**
    ...
    public function terms($query_args = [], $options = [])
    {
        ...
        // Defaults.
        $query_args = \wp_parse_args($query_args, ['taxonomy' => 'all']);
        $options = \wp_parse_args($options, ['merge' => \true]);
        ...
        $query = \array_merge($query_args, ['object_ids' => [$this->ID], 'taxonomy' => $taxonomies]);
        if (!$merge) {
    ...
    {
        return !$this->meta($field_name) ? \false : \true;
    }
    /**
     ...
     * @inheritDoc
     */
    protected function fetch_meta($field_name = '', $args = [], $apply_filters = \true)
    {
    ...
    {
        Helper::deprecated("{{ post.get_field('field_name') }}", "{{ post.meta('field_name') }}", '2.0.0');
        if ($field_name === null) {
    ...
    {
        Helper::deprecated("Importing field data onto an object", "{{ post.meta('field_name') }}", '2.0.0');
    ...
    {
        $user_id = \get_post_meta($this->ID, '_edit_last', \true);
    ...
    public function next($in_same_term = \false)
    {
        if (!isset($this->_next) || !isset($this->_next[$in_same_term])) {
            ...
            if (\is_string($in_same_term) && \strlen($in_same_term)) {
                $adjacent = \get_adjacent_post(\true, '', \false, $in_same_term);
            } else {
    ...
    public function pagination()
    {
        ...
        if ($multipage) {
            for ($i = 1; $i <= $numpages; $i++) {
                ...
                $data = ['name' => $i, 'title' => $i, 'text' => $i, 'link' => $link];
                if ($i == $page) {
                    $data['current'] = \true;
                }
                $ret['pages'][] = $data;
    ...
    public function prev($in_same_term = \false)
    {
        ...
        $adjacent = \get_adjacent_post($in_same_term, '', \true, $within_taxonomy);
    ...
    {
        return (int) \get_post_meta($this->ID, '_thumbnail_id', \true);
    }
    /**
     ...
     * expanded shortcode.
     */
    public function gallery($html = \true)
    {
    ...
    private function partition_tax_queries(array $query, array $taxonomies) : array
    {
        return \array_map(function (string $tax) use($query) : array {
            return \array_merge($query, ['taxonomy' => [$tax]]);
        }, $taxonomies);
    }
    /**
