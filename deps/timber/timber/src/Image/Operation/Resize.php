...
namespace Mwf\Lib\Deps\Timber\Image\Operation;
...
class Resize extends ImageOperation
{
    ...
    public function filename($src_filename, $src_extension)
    {
        ...
        $result = $src_filename . '-' . $w . 'x' . $h . '-c-' . ($this->crop ? $this->crop : 'f');
        // Crop will be either user named or f (false)
        if ($src_extension) {
    ...
    protected function run_animated_gif($load_filename, $save_filename, WP_Image_Editor $editor)
    {
        $w = $this->w;
        $h = $this->h;
        if (!\class_exists('Imagick') || \defined('Mwf\\Lib\\Deps\\TEST_NO_IMAGICK') && TEST_NO_IMAGICK) {
        ...
        return $image->writeImages($save_filename, \true);
    }
    /**
    ...
    protected function get_target_sizes(WP_Image_Editor $image)
    {
        ...
        if (!$crop || $crop === 'default') {
            return ['x' => 0, 'y' => 0, 'src_w' => $src_w, 'src_h' => $src_h, 'target_w' => $w, 'target_h' => $h];
        }
        // Get ratios
        $dest_ratio = $w / $h;
        ...
        // Crop the image
        return $dest_ratio > $src_ratio ? ['x' => 0, 'y' => $src_y, 'src_w' => $src_w, 'src_h' => $src_ht, 'target_w' => $w, 'target_h' => $h] : ['x' => $src_x, 'y' => 0, 'src_w' => $src_wt, 'src_h' => $src_h, 'target_w' => $w, 'target_h' => $h];
    }
    /**
