...
use Mwf\Lib\Deps\Timber\Attachment;
use Mwf\Lib\Deps\Timber\CoreInterface;
use Mwf\Lib\Deps\Timber\Helper;
use Mwf\Lib\Deps\Timber\Image;
use Mwf\Lib\Deps\Timber\PathHelper;
...
class PostFactory
{
    ...
    protected function from_post_object(object $obj) : CoreInterface
    {
        ...
        throw new InvalidArgumentException(\sprintf('Expected an instance of Timber\\CoreInterface or WP_Post, got %s', \get_class($obj)));
    }
    ...
    protected function get_post_class(WP_Post $post) : string
    {
         ...
         */
        if ('deprecated' !== \apply_filters('Mwf\\Lib\\Deps\\Timber\\PostClassMap', 'deprecated')) {
            Helper::doing_it_wrong('The `Timber\\PostClassMap` filter', 'Use the `timber/post/classmap` filter instead.', '2.0.0');
        }
        /**
    ...
    protected function is_image(WP_Post $post)
    {
        ...
        $extensions = \apply_filters('timber/post/image_extensions', ['jpg', 'jpeg', 'jpe', 'gif', 'png', 'svg', 'webp']);
