...
namespace Mwf\Lib\Deps\Timber\Factory;
...
class TermFactory
{
    public function from($params)
    {
        ...
        if (\is_array($params)) {
            return $this->from_wp_term_query(new WP_Term_Query($this->filter_query_params($params)));
        }
        return null;
    }
    ...
    protected function from_term_object(object $obj) : CoreInterface
    {
        ...
        throw new InvalidArgumentException(\sprintf('Expected an instance of Timber\\CoreInterface or WP_Term, got %s', \get_class($obj)));
    }
    protected function from_taxonomy_names(array $names)
    {
        return $this->from_wp_term_query(new WP_Term_Query($this->filter_query_params(['taxonomy' => $names])));
    }
    protected function get_term_class(WP_Term $term) : string
    {
        // Get the user-configured Class Map
        $map = \apply_filters('timber/term/classmap', ['post_tag' => Term::class, 'category' => Term::class]);
    ...
    {
        $corrections = ['taxonomies' => 'taxonomy', 'taxs' => 'taxonomy', 'tax' => 'taxonomy'];
    ...
    protected function correct_taxonomies($tax) : array
    {
        ...
        $corrections = ['categories' => 'category', 'tags' => 'post_tag', 'tag' => 'post_tag'];
