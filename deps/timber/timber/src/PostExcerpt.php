...
class PostExcerpt
{
    ...
    {
        $this->post = $post;
        $defaults = ['words' => 50, 'chars' => \false, 'end' => '&hellip;', 'force' => \false, 'strip' => \true, 'read_more' => 'Read More', 'always_add_read_more' => \false, 'always_add_end' => \false];
        /**
     ...
     * @return \Timber\PostExcerpt
     */
    public function force($force = \true)
    {
        $this->force = $force;
        return $this;
    }
     ...
     * @return \Timber\PostExcerpt
     */
    public function strip($strip = \true)
    {
        $this->strip = $strip;
        return $this;
    }
    ...
    protected function assemble($text, $args = [])
    {
        ...
        if ($this->read_more && ($this->always_add_read_more || $args['add_read_more'])) {
             ...
             * @since 1.0.4
             */
            $read_more_class = \apply_filters_deprecated('timber/post/preview/read_more_class', [$read_more_class], '2.0.0', 'timber/post/excerpt/read_more_class');
            $linktext = \trim($this->read_more);
            $link = \sprintf(' <a href="%1$s" class="%2$s">%3$s</a>', $this->post->link(), $read_more_class, $linktext);
            /**
             ...
             */
            $link = \apply_filters('timber/post/excerpt/read_more_link', $link, $this->post, $linktext, $read_more_class);
            /**
             * Filters the link used for a read more text in an excerpt.
             *
             * @deprecated 2.0.0
             ...
             * @ticket #1142
             */
            $link = \apply_filters_deprecated('timber/post/get_preview/read_more_link', [$link], '2.0.0', 'timber/post/excerpt/read_more_link');
            $text .= $link;
        }
    ...
    protected function run()
    {
        ...
        if (empty($text)) {
            ...
            $has_trimmed_chars = !empty($text_before_char_trim) && \strlen($text) < \strlen($text_before_char_trim);
        ...
        if (!empty($text)) {
            return $this->assemble($text, ['add_end' => $add_end, 'add_read_more' => $add_read_more]);
        }
        return \trim($text);
    }
}
