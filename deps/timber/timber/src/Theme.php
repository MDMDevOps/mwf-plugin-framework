...
class Theme extends Core implements JsonSerializable
{
    ...
    public function path()
    {
        ...
        return URLHelper::get_rel_url($this->link(), \true);
    }
    /**
     * @api
     * @param string $name
    ...
    {
        return ['name' => $this->name, 'parent' => ['name' => $this->parent->name, 'parent' => null, 'parent_slug' => null, 'slug' => $this->parent->slug, 'uri' => $this->parent->uri, 'version' => $this->parent->version], 'parent_slug' => $this->parent_slug, 'slug' => $this->slug, 'uri' => $this->uri, 'version' => $this->version];
    }
}
