...
class Menu extends CoreEntity
{
    ...
    public static function build(?WP_Term $menu, $args = []) : ?self
    {
         ...
         * @see wp_nav_menu()
         */
        $defaults = ['menu' => '', 'container' => 'div', 'container_class' => '', 'container_id' => '', 'container_aria_label' => '', 'menu_class' => 'menu', 'menu_id' => '', 'echo' => \true, 'fallback_cb' => 'wp_page_menu', 'before' => '', 'after' => '', 'link_before' => '', 'link_after' => '', 'items_wrap' => '<ul id="%1$s" class="%2$s">%3$s</ul>', 'item_spacing' => 'preserve', 'depth' => 0, 'walker' => '', 'theme_location' => ''];
        $args = \wp_parse_args($args, $defaults);
        if (!\in_array($args['item_spacing'], ['preserve', 'discard'], \true)) {
        ...
        foreach ((array) $menu_items as $menu_item) {
            ...
            if ($menu_item->menu_item_parent) {
                $menu_items_with_children[$menu_item->menu_item_parent] = \true;
            }
        }
    ...
    {
        Helper::deprecated("{{ menu.get_field('field_name') }}", "{{ menu.meta('field_name') }}", '2.0.0');
    ...
    public function current_item($depth = null)
    {
        ...
        if (!isset($this->_current_item)) {
            $current = $this->traverse_items_for_current($this->items, $depth);
            if (\is_null($depth)) {
    ...
    public function __toString()
    {
        ...
        if ($args->container) {
            ...
            if (\is_string($args->container) && \in_array($args->container, $allowed_tags, \true)) {
                $show_container = \true;
        ...
        if (!empty($args->menu_id)) {
            ...
            while (\in_array($wrap_id, $menu_id_slugs, \true)) {
