...
use Mwf\Lib\Deps\Timber\Factory\CommentFactory;
use Mwf\Lib\Deps\Timber\Factory\MenuFactory;
use Mwf\Lib\Deps\Timber\Factory\PagesMenuFactory;
...
class Timber
{
    ...
    {
        \defined("Mwf\\Lib\\Deps\\TIMBER_LOC") or \define("Mwf\\Lib\\Deps\\TIMBER_LOC", \realpath(\dirname(__DIR__)));
    }
    /**
     * @codeCoverageIgnore
     */
    public static function init()
    {
         ...
         */
        \class_alias('Mwf\\Lib\\Deps\\Timber\\Timber', 'Mwf\\Lib\\Deps\\Timber');
        \define('Mwf\\Lib\\Deps\\TIMBER_LOADED', \true);
    }
    /**
    ...
    {
        $integrations = [new Integration\AcfIntegration(), new Integration\CoAuthorsPlusIntegration(), new Integration\WpCliIntegration(), new Integration\WpmlIntegration()];
        /**
    ...
    public static function get_post($query = \false, $options = [])
    {
        if (\is_string($query) && !\is_numeric($query)) {
            Helper::doing_it_wrong('Timber::get_post()', 'Getting a post by post slug or post name was removed from Timber::get_post() in Timber 2.0. Use Timber::get_post_by() instead.', '2.0.0');
        }
        if (\is_string($options)) {
            Helper::doing_it_wrong('Timber::get_post()', 'The $PostClass parameter for passing in the post class to use in Timber::get_posts() was replaced with an $options array in Timber 2.0. To customize which class to instantiate for your post, use Class Maps instead: https://timber.github.io/docs/v2/guides/class-maps/', '2.0.0');
            $options = [];
        }
        $factory = new PostFactory();
        global $wp_query;
        $options = \wp_parse_args($options, ['merge_default' => \false]);
    ...
    {
        $args = \wp_parse_args($args, ['alt' => '', 'caption' => '']);
    ...
    {
        if (\is_string($query)) {
            Helper::doing_it_wrong('Timber::get_posts()', "Querying posts by using a query string was removed in Timber 2.0. Pass in the query string as an options array instead. For example, change Timber::get_posts( 'post_type=portfolio&posts_per_page=3') to Timber::get_posts( [ 'post_type' => 'portfolio', 'posts_per_page' => 3 ] ). Learn more: https://timber.github.io/docs/v2/reference/timber-timber/#get_posts", '2.0.0');
            $query = new WP_Query($query);
        }
        if (\is_string($options)) {
            Helper::doing_it_wrong('Timber::get_posts()', 'The $PostClass parameter for passing in the post class to use in Timber::get_posts() was replaced with an $options array in Timber 2.0. To customize which class to instantiate for your post, use Class Maps instead: https://timber.github.io/docs/v2/guides/class-maps/', '2.0.0');
            $options = [];
        }
        if (3 === \func_num_args()) {
            Helper::doing_it_wrong('Timber::get_posts()', 'The $return_collection parameter to control whether a post collection is returned in Timber::get_posts() was removed in Timber 2.0.', '2.0.0');
        }
        if (\is_array($query) && isset($query['numberposts'])) {
            Helper::doing_it_wrong('Timber::get_posts()', 'Using `numberposts` only works when using `get_posts()`, but not for Timber::get_posts(). Use `posts_per_page` instead.', '2.0.0');
        }
        /**
    ...
    {
        $post_id = \false;
        $args = \wp_parse_args($args, ['post_type' => 'any', 'order_by' => 'post_date', 'order' => 'ASC']);
        if ('slug' === $type) {
            $args = \wp_parse_args($args, ['name' => $search_value, 'fields' => 'ids']);
    ...
    public static function get_attachment_by(string $field_or_ident, string $ident = '')
    {
        if ($field_or_ident === 'url') {
            if (empty($ident)) {
                Helper::doing_it_wrong('Timber::get_attachment_by()', 'Passing "url" as the first arg requires passing a URL as the second arg.', '2.0.0');
                return null;
            }
        ...
        if ($field_or_ident === 'path') {
            if (empty($ident)) {
                Helper::doing_it_wrong('Timber::get_attachment_by()', 'Passing "path" as the first arg requires passing an absolute path as the second arg.', '2.0.0');
                return null;
            }
    ...
    public static function get_terms($args = null, array $options = []) : iterable
    {
        ...
        $args = $args ?? ['taxonomy' => \get_taxonomies()];
    ...
    public static function context_global()
    {
        if (empty(self::$context_cache)) {
             ...
             */
            self::$context_cache = \apply_filters_deprecated('timber_context', [self::$context_cache], '2.0.0', 'timber/context');
        }
    ...
    {
        if (!\defined('Mwf\\Lib\\Deps\\TIMBER_LOADED')) {
            self::init();
        }
        ...
        if ($via_render) {
             ...
             */
            $file = \apply_filters_deprecated('timber_render_file', [$file], '2.0.0', 'timber/render/file');
        } else {
            /**
             ...
             * @deprecated 2.0.0
             */
            $file = \apply_filters_deprecated('timber_compile_file', [$file], '2.0.0', 'timber/compile/file');
        }
        $output = \false;
        if ($file !== \false) {
            if (\is_null($data)) {
            ...
            if ($via_render) {
                 ...
                 * @deprecated 2.0.0
                 */
                $data = \apply_filters_deprecated('timber_render_data', [$data], '2.0.0', 'timber/render/data');
            } else {
                /**
                 ...
                 */
                $data = \apply_filters_deprecated('timber_compile_data', [$data], '2.0.0', 'timber/compile/data');
            }
    ...
    {
        Helper::deprecated('fetch', 'Timber::compile() (see https://timber.github.io/docs/v2/reference/timber/#compile for more information)', '2.0.0');
        $output = self::compile($filenames, $data, $expires, $cache_mode, \true);
        /**
         ...
         */
        $output = \apply_filters_deprecated('timber_compile_result', [$output], '2.0.0', 'timber/compile/result');
        return $output;
    }
    /**
     * Renders a Twig file.
    ...
    {
        Helper::deprecated('get_pagination', '{{ posts.pagination }} (see https://timber.github.io/docs/v2/guides/pagination/ for more information)', '2.0.0');
