...
class Archives extends Core
{
     ...
     * @return array
     */
    protected function get_items_monthly($args, $last_changed, $join, $where, $order, $limit = '', $nested = \true)
    {
        global $wpdb, $wp_locale;
        $output = [];
        $defaults = ['show_year' => \false];
        ...
        if ($nested) {
            ...
            foreach ($output as $year => $months) {
                $out2[] = ['name' => $year, 'children' => $months, 'post_count' => \array_sum(\array_column($months, 'post_count'))];
            }
            return $out2;
        }
        return $output;
    ...
    {
        global $wpdb;
        $defaults = ['type' => 'monthly-nested', 'limit' => '', 'show_post_count' => \false, 'order' => 'DESC', 'post_type' => 'post', 'show_year' => \false, 'nested' => \false];
        if ($args === null) {
            $args = $this->args;
        }
