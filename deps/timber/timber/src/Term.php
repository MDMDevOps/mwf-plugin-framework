...
class Term extends CoreEntity
{
    ...
    {
        Helper::deprecated("{{ term.get_meta_field('field_name') }}", "{{ term.meta('field_name') }}", '2.0.0');
    ...
    public function link()
    {
         ...
         */
        $link = \apply_filters_deprecated('timber_term_link', [$link, $this], '2.0.0', 'timber/term/link');
        return $link;
    }
    /**
     * Gets a term meta value.
    ...
    {
        Helper::deprecated("{{ term.get_field('field_name') }}", "{{ term.meta('field_name') }}", '2.0.0');
        return $this->meta($field_name);
    }
    /**
    ...
    {
        $link = $this->link();
        $rel = URLHelper::get_rel_url($link, \true);
        /**
         ...
         */
        $rel = \apply_filters_deprecated('timber_term_path', [$rel, $this], '2.0.0', 'timber/term/path');
        return $rel;
    }
    /**
    ...
    {
        if (\is_string($query)) {
            Helper::doing_it_wrong('Passing a query string to Term::posts()', 'Pass a query array instead: e.g. `"posts_per_page=3"` should be replaced with `["posts_per_page" => 3]`', '2.0.0');
            return \false;
        }
        if (\is_int($query)) {
            $query = ['posts_per_page' => $query, 'post_type' => 'any'];
        }
        if (isset($post_type_or_class)) {
            Helper::deprecated('Passing post_type_or_class', 'Pass post_type as part of the $query argument. For specifying class, use Class Maps: https://timber.github.io/docs/v2/guides/class-maps/', '2.0.0');
        ...
        if (\func_num_args() > 2) {
            Helper::doing_it_wrong('Passing a post class', 'Use Class Maps instead: https://timber.github.io/docs/v2/guides/class-maps/', '2.0.0');
        }
        $tax_query = [
            // Force a tax_query constraint on this term.
            'relation' => 'AND',
            ['field' => 'id', 'terms' => $this->ID, 'taxonomy' => $this->taxonomy],
        ];
    ...
    public function update($key, $value)
    {
         ...
         */
        $value = \apply_filters_deprecated('timber_term_set_meta', [$value, $key, $this->ID, $this], '2.0.0', \false, 'This filter will be removed in a future version of Timber. There is no replacement.');
        /**
         * Filters term meta value that is going to be updated.
         *
         ...
         * @deprecated 2.0.0, with no replacement
         */
        $value = \apply_filters_deprecated('timber/term/meta/set', [$value, $key, $this->ID, $this], '2.0.0', \false, 'This filter will be removed in a future version of Timber. There is no replacement.');
        $this->{$key} = $value;
    }
}
