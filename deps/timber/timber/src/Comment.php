...
class Comment extends CoreEntity
{
    ...
    public function author()
    {
        if ($this->user_id) {
            ...
            return (object) ['name' => $this->comment_author ?: 'Anonymous'];
        }
    }
    /**
    ...
    public function avatar($size = 92, $default = '')
    {
        ...
        $args = ['size' => $size, 'default' => $default];
    ...
    {
        Helper::deprecated("{{ comment.get_meta_field('field_name') }}", "{{ comment.meta('field_name') }}", '2.0.0');
    ...
    {
        Helper::deprecated("{{ comment.get_field('field_name') }}", "{{ comment.meta('field_name') }}", '2.0.0');
        return $this->meta($field_name);
    }
    /**
    ...
    public function reply_link($reply_text = 'Reply')
    {
        ...
        // Default args
        $args = ['add_below' => 'comment', 'respond_id' => 'respond', 'reply_text' => $reply_text, 'depth' => $this->depth() + 1, 'max_depth' => $max_depth];
