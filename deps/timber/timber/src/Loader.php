...
use Mwf\Lib\Deps\Twig\CacheExtension;
use Mwf\Lib\Deps\Twig\Environment;
use Mwf\Lib\Deps\Twig\Loader\FilesystemLoader;
...
class Loader
{
    ...
    public function __construct($caller = \false)
    {
         ...
         */
        $this->cache_mode = \apply_filters_deprecated('timber_cache_mode', [$this->cache_mode], '2.0.0', 'timber/cache/mode');
    }
    /**
     * @param string            $file
     * @param array             $data
    ...
    public function render($file, $data = null, $expires = \false, $cache_mode = self::CACHE_USE_DEFAULT)
    {
        ...
        if (\false === $output || null === $output) {
            ...
            if (\strlen($file)) {
                 ...
                 */
                \do_action_deprecated('timber_loader_render_file', [$result], '2.0.0', 'timber/loader/render_file');
            }
            /**
             * Filters …
             *
             ...
             */
            $data = \apply_filters_deprecated('timber_loader_render_data', [$data], '2.0.0', 'timber/loader/render_data');
    ...
    public function get_loader()
    {
         ...
         * @param array $paths
         */
        $paths = \apply_filters_deprecated('timber/loader/paths', [$paths], '2.0.0', 'timber/locations');
        ...
        foreach ($paths as $namespace => $path_locations) {
            if (\is_array($path_locations)) {
                ...
                Helper::deprecated('add_filter( \'timber/loader/paths\', [\'path/to/my/templates\'] ) in a non-associative array', 'add_filter( \'timber/loader/paths\', [ 0 => [ \'path/to/my/templates\' ] ] )', '2.0.0');
    ...
    public function get_twig()
    {
         ...
         */
        $environment_options = \apply_filters('timber/twig/environment/options', $environment_options);
        /**
         * @deprecated 2.0.0
         */
        if (isset(Timber::$autoescape) && \false !== Timber::$autoescape) {
            Helper::deprecated('Timber::$autoescape', 'the \'timber/twig/environment/options filter\'', '2.0.0');
         ...
         */
        if (\true === $environment_options['autoescape']) {
         ...
         * @deprecated 2.0.0
         */
        if (isset(Timber::$cache) && \true === Timber::$cache) {
            Timber::$twig_cache = \true;
        }
        /**
         * @deprecated 2.0.0
         */
        if (isset(Timber::$twig_cache) && \false !== Timber::$twig_cache) {
            Helper::deprecated('Timber::$cache and Timber::$twig_cache', 'the \'timber/twig/environment/options filter\'', '2.0.0');
            $environment_options['cache'] = Timber::$twig_cache;
        }
        if (\true === $environment_options['cache']) {
             ...
             */
            $twig_cache_loc = \apply_filters_deprecated('timber/cache/location', [$twig_cache_loc], '2.0.0', 'timber/twig/environment/options');
            if (!\file_exists($twig_cache_loc)) {
                \mkdir($twig_cache_loc, 0777, \true);
            }
         ...
         */
        $enable_cache_extension = \apply_filters('timber/cache/enable_extension', \true);
        if ($enable_cache_extension && \class_exists('Mwf\\Lib\\Deps\\Twig\\CacheExtension\\Extension')) {
    ...
    {
        global $wpdb;
        return $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->options} WHERE option_name LIKE '%s'", '_transient%timberloader_%'));
    }
    /**
    ...
    public function clear_cache_twig()
    {
        ...
        $cache_location = $twig->getCache(\true);
        // Cache not activated.
        if (!$cache_location) {
            return \true;
        }
    ...
    {
        $key_generator = new \Mwf\Lib\Deps\Timber\Cache\KeyGenerator();
        $cache_provider = new \Mwf\Lib\Deps\Timber\Cache\WPObjectCacheAdapter($this);
