...
class Pagination
{
    ...
    protected function init($prefs = [], $wp_query = null)
    {
        ...
        if (isset($current) && isset($this->pages[$current + 1]) && isset($this->pages[$current + 1]['link'])) {
            $this->next = ['link' => $this->pages[$current + 1]['link'], 'class' => 'page-numbers next'];
        }
        if (isset($current) && isset($this->pages[$current - 1]) && isset($this->pages[$current - 1]['link'])) {
            $this->prev = ['link' => $this->pages[$current - 1]['link'], 'class' => 'page-numbers prev'];
        }
        if ($paged < 2) {
            $this->prev = '';
        }
    ...
    public static function paginate_links($args = [])
    {
        ...
        $args['start_size'] = 0 <= (int) $args['start_size'] ? (int) $args['start_size'] : $args['end_size'];
        // Default to end_size for backwards compat
        ...
        for ($n = 1; $n <= $args['total']; $n++) {
            $n_display = \number_format_i18n($n);
            if ($n == $args['current']) {
                $page_links[] = ['class' => 'page-number page-numbers current', 'title' => $n_display, 'text' => $n_display, 'name' => $n_display, 'current' => \true];
                $dots = \true;
            } else {
                if ($args['show_all'] || ($n <= (int) $args['start_size'] || $args['current'] && $n >= (int) $args['current'] - (int) $args['mid_size'] && $n <= (int) $args['current'] + (int) $args['mid_size'] || $n > (int) $args['total'] - (int) $args['end_size'])) {
                    ...
                    $page_links[] = ['class' => 'page-number page-numbers', 'link' => \esc_url($link), 'title' => $n_display, 'name' => $n_display, 'current' => $args['current'] == $n];
                    $dots = \true;
                } elseif ($dots && !$args['show_all']) {
                    $page_links[] = ['class' => 'dots', 'title' => \__('&hellip;')];
                    $dots = \false;
                }
            }
        }
