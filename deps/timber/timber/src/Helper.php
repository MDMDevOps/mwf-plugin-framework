...
class Helper
{
    ...
    public static function transient($slug, $callback, $transient_time = 0, $lock_timeout = 5, $force = \false)
    {
        ...
        $enable_transients = $transient_time === \false || \defined('Mwf\\Lib\\Deps\\WP_DISABLE_TRANSIENTS') && WP_DISABLE_TRANSIENTS ? \false : \true;
    ...
    protected static function handle_transient_locking($slug, $callback, $transient_time, $lock_timeout, $force, $enable_transients)
    {
        if ($enable_transients && self::_is_transient_locked($slug)) {
             ...
             */
            $force = \apply_filters_deprecated('timber_force_transients', [$force], '2.0.0', 'timber/transient/force_transients');
            /**
             * Filters …
             *
    ...
    {
        \set_transient($slug . '_lock', \true, $lock_timeout);
    }
    /**
     * @internal
     * @param string $slug
    ...
    public static function doing_it_wrong($function, $message, $version)
    {
         ...
         * @param string $version  The version of WordPress where the message was added.
         */
        $should_trigger_error = \apply_filters('doing_it_wrong_trigger_error', \true, $function, $message, $version);
        if ($should_trigger_error) {
            if (\is_null($version)) {
                $version = '';
            } else {
                $version = \sprintf('(This message was added in Timber version %s.)', $version);
            }
            $message .= \sprintf(' Please see Debugging in WordPress (%1$s) as well as Debugging in Timber (%2$s) for more information.', 'https://wordpress.org/support/article/debugging-in-wordpress/', 'https://timber.github.io/docs/v2/guides/debugging/');
            $error_message = \sprintf('%1$s was called <strong>incorrectly</strong>. %2$s %3$s', $function, $message, $version);
    ...
    public static function deprecated($function, $replacement, $version)
    {
         ...
         */
        if (!\apply_filters('deprecated_function_trigger_error', \true)) {
            return;
        }
        if (!\is_null($replacement)) {
            $error_message = \sprintf('%1$s is deprecated since Timber version %2$s! Use %3$s instead.', $function, $version, $replacement);
        } else {
            $error_message = \sprintf('%1$s is deprecated since Timber version %2$s with no alternative available.', $function, $version);
        }
        // phpcs:disable WordPress.PHP.DevelopmentFunctions.error_log_trigger_error
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
        \trigger_error('[ Timber ] ' . $error_message);
    ...
    public static function is_true($value)
    {
        if (isset($value)) {
            ...
            if (($value == 'true' || $value === 1 || $value === '1' || $value == \true) && $value !== \false && $value !== 'false') {
                return \true;
            }
        }
        return \false;
