...
class PagesMenu extends Menu
{
    ...
    public static function build($menu, $args = []) : ?self
    {
         ...
         * @see wp_page_menu()
         */
        $defaults = ['sort_column' => 'menu_order, post_title', 'echo' => \true, 'link_before' => '', 'link_after' => '', 'before' => '<ul>', 'after' => '</ul>', 'item_spacing' => 'discard', 'walker' => '', 'menu_id' => '', 'menu_class' => 'menu', 'container' => 'div'];
         ...
         * @see wp_list_pages()
         */
        $defaults = ['depth' => 0, 'show_date' => '', 'date_format' => \get_option('date_format'), 'child_of' => 0, 'exclude' => '', 'title_li' => \__('Pages'), 'echo' => 1, 'authors' => '', 'sort_column' => 'menu_order, post_title', 'link_before' => '', 'link_after' => '', 'item_spacing' => 'preserve', 'walker' => ''];
        $args = \wp_parse_args($args, $defaults);
        if (!\in_array($args['item_spacing'], ['preserve', 'discard'], \true)) {
            // Invalid value, fall back to default.
            $args['item_spacing'] = $defaults['item_spacing'];
        ...
        $exclude_array = $args['exclude'] ? \explode(',', $args['exclude']) : [];
        /**
