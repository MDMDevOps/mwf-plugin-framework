...
class Twig
{
    ...
    public function get_timber_functions()
    {
        ...
        $functions = [
            'action' => ['callable' => function ($action_name, ...$args) {
                \do_action_ref_array($action_name, $args);
            }],
            'function' => ['callable' => [$this, 'exec_function']],
            'fn' => ['callable' => [$this, 'exec_function']],
            'get_post' => ['callable' => [Timber::class, 'get_post']],
            'get_image' => ['callable' => [Timber::class, 'get_image']],
            'get_external_image' => ['callable' => [Timber::class, 'get_external_image']],
            'get_attachment' => ['callable' => [Timber::class, 'get_attachment']],
            'get_posts' => ['callable' => [Timber::class, 'get_posts']],
            'get_attachment_by' => ['callable' => [Timber::class, 'get_attachment_by']],
            'get_term' => ['callable' => [Timber::class, 'get_term']],
            'get_terms' => ['callable' => [Timber::class, 'get_terms']],
            'get_user' => ['callable' => [Timber::class, 'get_user']],
            'get_users' => ['callable' => [Timber::class, 'get_users']],
            'get_comment' => ['callable' => [Timber::class, 'get_comment']],
            'get_comments' => ['callable' => [Timber::class, 'get_comments']],
            'Post' => ['callable' => function ($post_id) use($post_factory) {
                Helper::deprecated('{{ Post() }}', '{{ get_post() }} or {{ get_posts() }}', '2.0.0');
                return $post_factory->from($post_id);
            }, 'options' => ['deprecated' => \true]],
            'TimberPost' => ['callable' => function ($post_id) use($post_factory) {
                Helper::deprecated('{{ TimberPost() }}', '{{ get_post() }} or {{ get_posts() }}', '2.0.0');
                return $post_factory->from($post_id);
            }, 'options' => ['deprecated' => \true]],
            'Image' => ['callable' => function ($post_id) use($post_factory) {
                Helper::deprecated('{{ Image() }}', '{{ get_image() }}, {{ get_post() }}, {{ get_posts() }}, {{ get_attachment() }} or {{ get_attachment_by() }}', '2.0.0');
                return $post_factory->from($post_id);
            }, 'options' => ['deprecated' => \true]],
            'TimberImage' => ['callable' => function ($post_id) use($post_factory) {
                Helper::deprecated('{{ TimberImage() }}', '{{ get_image() }}, {{ get_post() }}, {{ get_posts() }}, {{ get_attachment() }} or {{ get_attachment_by() }}', '2.0.0');
                return $post_factory->from($post_id);
            }, 'options' => ['deprecated' => \true]],
            'Term' => ['callable' => function ($term_id) use($termFactory) {
                Helper::deprecated('{{ Term() }}', '{{ get_term() }} or {{ get_terms() }}', '2.0.0');
                return $termFactory->from($term_id);
            }, 'options' => ['deprecated' => \true]],
            'TimberTerm' => ['callable' => function ($term_id) use($termFactory) {
                Helper::deprecated('{{ TimberTerm() }}', '{{ get_term() }} or {{ get_terms() }}', '2.0.0');
                return $termFactory->from($term_id);
            }, 'options' => ['deprecated' => \true]],
            'User' => ['callable' => function ($user_id) {
                Helper::deprecated('{{ User() }}', '{{ get_user() }} or {{ get_users() }}', '2.0.0');
                return Timber::get_user($user_id);
            }, 'options' => ['deprecated' => \true]],
            'TimberUser' => ['callable' => function ($user_id) {
                Helper::deprecated('{{ TimberUser() }}', '{{ get_user() }} or {{ get_users() }}', '2.0.0');
                return Timber::get_user($user_id);
            }, 'options' => ['deprecated' => \true]],
            'shortcode' => ['callable' => 'do_shortcode'],
            'bloginfo' => ['callable' => 'bloginfo'],
            ...
            '_ex' => ['callable' => '_ex'],
            '_nx' => ['callable' => '_nx'],
            '_n_noop' => ['callable' => '_n_noop'],
            '_nx_noop' => ['callable' => '_nx_noop'],
            'translate_nooped_plural' => ['callable' => 'translate_nooped_plural'],
        ];
        /**
    ...
    public function add_timber_functions($twig)
    {
        foreach ($this->get_timber_functions() as $name => $function) {
            $twig->addFunction(new TwigFunction($name, $function['callable'], $function['options'] ?? []));
        }
        return $twig;
    }
    /**
    ...
    public function get_timber_filters()
    {
        $filters = [
            /* image filters */
            'resize' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\ImageHelper', 'resize']],
            'retina' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\ImageHelper', 'retina_resize']],
            'letterbox' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\ImageHelper', 'letterbox']],
            'tojpg' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\ImageHelper', 'img_to_jpg']],
            'towebp' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\ImageHelper', 'img_to_webp']],
            // Debugging filters.
            'get_class' => ['callable' => function ($obj) {
                Helper::deprecated('{{ my_object | get_class }}', "{{ function('get_class', my_object) }}", '2.0.0');
                return \get_class($obj);
            }, 'options' => ['deprecated' => \true]],
            'print_r' => ['callable' => function ($arr) {
                Helper::deprecated('{{ my_object | print_r }}', '{{ dump(my_object) }}', '2.0.0');
                return \print_r($arr, \true);
            }, 'options' => ['deprecated' => \true]],
            // Other filters.
            'stripshortcodes' => ['callable' => 'strip_shortcodes'],
            'array' => ['callable' => [$this, 'to_array']],
            'excerpt' => ['callable' => 'wp_trim_words'],
            'excerpt_chars' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\TextHelper', 'trim_characters']],
            'function' => ['callable' => [$this, 'exec_function']],
            'pretags' => ['callable' => [$this, 'twig_pretags']],
            'sanitize' => ['callable' => 'sanitize_title'],
            'shortcodes' => ['callable' => 'do_shortcode'],
            'wpautop' => ['callable' => 'wpautop'],
            'list' => ['callable' => [$this, 'add_list_separators']],
            'pluck' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\Helper', 'pluck']],
            'wp_list_filter' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\Helper', 'wp_list_filter']],
            'relative' => ['callable' => function ($link) {
                return URLHelper::get_rel_url($link, \true);
            }],
            /**
             * Date and Time filters.
             */
            'date' => ['callable' => [$this, 'twig_date_format_filter'], 'options' => ['needs_environment' => \true]],
            'time_ago' => ['callable' => ['Mwf\\Lib\\Deps\\Timber\\DateTimeHelper', 'time_ago']],
            'truncate' => ['callable' => function ($text, $len) {
                return TextHelper::trim_words($text, $len);
            }],
            // Numbers filters
            'size_format' => ['callable' => 'size_format'],
            // Actions and filters.
            'apply_filters' => ['callable' => function () {
                $args = \func_get_args();
    ...
    public function add_timber_filters($twig)
    {
        foreach ($this->get_timber_filters() as $name => $function) {
            $twig->addFilter(new TwigFilter($name, $function['callable'], $function['options'] ?? []));
        }
        return $twig;
    }
    /**
    ...
    public function add_timber_escapers($twig)
    {
        ...
        if (\class_exists('Mwf\\Lib\\Deps\\Twig\\Extension\\EscaperExtension')) {
            $escaper_extension = $twig->getExtension('Mwf\\Lib\\Deps\\Twig\\Extension\\EscaperExtension');
