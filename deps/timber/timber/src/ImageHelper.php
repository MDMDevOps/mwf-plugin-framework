...
use Mwf\Lib\Deps\Timber\Image\Operation;
/**
 * Class ImageHelper
 *
...
class ImageHelper
{
    ...
    public static function is_svg($file_path)
    {
         ...
         * type for SVG images.
         */
        $mime = \wp_check_filetype_and_ext($file_path, PathHelper::basename($file_path), ['svg' => 'image/svg+xml']);
        return \in_array($mime['type'], ['image/svg+xml', 'text/html', 'text/plain', 'image/svg']);
    }
    /**
    ...
    private static function _operate($src, $op, $force = \false)
    {
        ...
        $allow_fs_write = \apply_filters('timber/allow_fs_write', \true);
